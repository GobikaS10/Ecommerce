...
export const getDocuments = async (collectionName) => {
  try {
    const response = await domo.get(
      `${BASE_URL}/collections/${collectionName}/documents`
    );

    return response; // returns array of documents
  } catch (error) {
...
export const createDocument = async (collectionName, document) => {
  try {
    const response = await domo.post(
      ...
      {
        content: document, // ✅ correct
      }
    );

    return response; // domo SDK already returns data
  } catch (error) {
    console.error(" Error creating document:", error);
    throw error;
  }
...
export const updateDocument = async (collectionName, documentId, document) => {
  try {
    const response = await domo.put(
      `${BASE_URL}/collections/${collectionName}/documents/${documentId}`,
      {
        content: document,
      }
    );
